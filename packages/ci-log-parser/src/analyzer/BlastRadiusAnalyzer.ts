...
export class BlastRadiusAnalyzer {
  ...
  static analyze(
    ...
    if (event.type === 'auth') {
      ...
      if (radius === 'low') radius = 'medium';
      if (radius === 'medium') radius = 'high';
    }
   ...
   */
  private static getRadiusForType(type: FailureType): BlastRadius {
    const radiusMap: Record<FailureType, BlastRadius> = {
      'build': 'high',
      'deploy': 'high',
      'auth': 'high',
      'test': 'medium',
   ...
   */
  private static isCriticalStep(step: string): boolean {
   ...
   */
  private static escalateRadius(current: BlastRadius): BlastRadius {
    const escalation: Record<BlastRadius, BlastRadius> = {
      'low': 'medium',
      'medium': 'high',
      'high': 'high'
    };
  ...
  private static generateReasoning(
    ...
    const baseReasons: Record<BlastRadius, string> = {
