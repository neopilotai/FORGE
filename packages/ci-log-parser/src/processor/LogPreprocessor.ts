...
export class LogPreprocessor {
   ...
   */
  static redact(logs: string): RedactionResult {
    let processed = logs;
    const redactedPatterns: Set<string> = new Set();
    let totalSecrets = 0;
   ...
   */
  static prune(logs: string, headLines: number = 100, tailLines: number = 500): {
    pruned: string;
    prunedLines: number;
    totalLines: number;
  } {
   ...
   */
  static process(logs: string, headLines?: number, tailLines?: number) {
